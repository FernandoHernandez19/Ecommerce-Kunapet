  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm mb-6">
      <h2 className="text-lg font-bold text-gray-900 flex items-center mb-5">
        <span className="text-[#006D44] mr-2">💵</span> Método de Pago
      </h2>
      ...
      <div className="space-y-3">
        {methods.map((method) => {
          ...
          return (
            <div key={method.id} className={`border rounded-xl transition-all overflow-hidden ${isSelected ? 'border-[#006D44] bg-emerald-50/10' : 'border-gray-200'}`}>
              
              {/* Cabecera del Radio Button */}
              <label className="flex items-center justify-between p-4 cursor-pointer group">
                <div className="flex items-center space-x-3">
                  <div className={`w-4 h-4 rounded-full border flex items-center justify-center ${isSelected ? 'border-[#006D44]' : 'border-gray-300'}`}>
                    {isSelected && <div className="w-2.5 h-2.5 rounded-full bg-[#006D44]"></div>}
                  </div>
              ...
              {isSelected && method.id === 'card' && (
                <div className="p-4 pt-0 border-t border-gray-100 animate-fade-in-down">
                  <div className="space-y-3 mt-4">
                    <input type="text" placeholder="Número de Tarjeta" className="w-full text-sm p-3 border border-gray-200 rounded-lg outline-none focus:border-[#006D44] focus:ring-1 focus:ring-[#006D44] transition-all" />
                    <div className="flex space-x-3">
                      <input type="text" placeholder="MM/AA" className="w-1/2 text-sm p-3 border border-gray-200 rounded-lg outline-none focus:border-[#006D44] transition-all" />
                      <input type="text" placeholder="CVV" className="w-1/2 text-sm p-3 border border-gray-200 rounded-lg outline-none focus:border-[#006D44] transition-all" />
                    </div>
                  </div>
                </div>
              )}
