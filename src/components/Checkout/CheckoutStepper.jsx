          ...
          return (
            <div key={step.id} className="flex flex-col items-center bg-[#FDFDFD] px-2 relative">
              ...
              {index > 0 && (isCompleted || isCurrent) && (
                <div className="absolute right-1/2 top-4 w-[200vw] h-0.5 bg-[#e23d28] -z-10"></div>
              )}
              ...
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold shadow-sm transition-all ${
                ...
                isCurrent ? 'bg-emerald-200 text-[#e23d28] ring-4 ring-red-50' : 
                'bg-gray-100 text-gray-400'
              }`}>
                {isCompleted ? '✓' : isCurrent ? '🐾' : step.id}
              </div>
              <span className={`text-xs font-bold mt-2 ${isCurrent ? 'text-[#e23d28]' : isCompleted ? 'text-gray-800' : 'text-gray-400'}`}>
                {step.label}
              </span>
            </div>
          );
