          ...
          <React.Fragment key={step.id}>
            <div className="flex flex-col items-center relative z-10">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-colors ${
                ...
                isCurrent ? 'bg-[#006D44] text-white ring-4 ring-emerald-100' : 
                'bg-gray-200 text-gray-500'
              }`}>
                {isCompleted ? '✓' : step.id}
              </div>
              <span className={`absolute -bottom-6 text-[11px] font-bold ${isCurrent || isCompleted ? 'text-[#006D44]' : 'text-gray-400'}`}>
                {step.label}
              </span>
            </div>
            {/* Línea conectora */}
            {index < steps.length - 1 && (
              <div className={`flex-1 h-1 mx-2 rounded-full transition-colors ${
                isCompleted ? 'bg-[#006D44]' : 'bg-gray-200'
              }`}></div>
            )}
          </React.Fragment>
        );
