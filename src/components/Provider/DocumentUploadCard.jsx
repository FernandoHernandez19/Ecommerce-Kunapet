...
export default function DocumentUploadCard({ id, title, description, isRequired, icon, file, onFileUpload }) {
  ...
  return (
    <div className={`bg-white rounded-2xl border p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 transition-all shadow-sm hover:shadow-md ${isUploaded ? 'border-emerald-200 bg-red-50/10' : 'border-gray-200'}`}>
      
      {/* Información del Documento */}
      <div className="flex items-start space-x-4">
        <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${isUploaded ? 'bg-red-100 text-brand-primary' : 'bg-gray-100 text-gray-500'}`}>
          {icon}
        </div>
        <div>
      ...
      <div className="flex items-center space-x-3 w-full md:w-auto justify-between md:justify-end mt-2 md:mt-0 pt-4 md:pt-0 border-t md:border-0 border-gray-50">
        ...
        {/* Etiqueta de Estado */}
        {isUploaded ? (
          <div className="flex items-center space-x-1.5 bg-red-50 px-3 py-1.5 rounded-full border border-red-100 max-w-[150px]">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 text-brand-primary flex-shrink-0"><path fillRule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12Zm13.36-1.814a.75.75 0 1 0-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 0 0-1.06 1.06l2.5 2.5a.75.75 0 0 0 1.11-.049l3.75-5.25Z" clipRule="evenodd" /></svg>
            <span className="text-xs font-bold text-brand-primary truncate" title={file.name}>{file.name}</span>
          </div>
        ) : (
        ...
        <button 
          ...
          className={`flex items-center space-x-2 px-4 py-2 border rounded-xl text-sm font-bold transition-colors ${
            isUploaded 
              ...
              : 'border-[#e23d28] text-[#e23d28] hover:bg-red-50'
          }`}
        >
