...
export default function ClientDashboard() {
  ...
  useEffect(() => {
    // Simulación de fetch de datos de la sesión del cliente
      ...
      <main className="flex-1 p-6 md:p-10 lg:ml-64">
        ...
        <header className="mb-8">
          <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">
            ¡Hola, {userData.clientName}!
          </h1>
          <p className="mt-2 text-sm text-gray-600 font-medium">
            Es un gran día para mimar a tus peludos. Tienes{' '}
            <span className="text-emerald-700 font-bold">{userData.scheduledServicesCount} servicio</span> programado para hoy.
          </p>
        </header>
              ...
              <button className="flex items-center justify-between p-4 bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md hover:border-gray-200 transition-all group text-left">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">{userData.paymentMethod.type} terminada en **** {userData.paymentMethod.lastFour}</p>
                  </div>
                </div>
                <span className="text-gray-400 group-hover:text-gray-600 transition-colors">→</span>
              </button>
